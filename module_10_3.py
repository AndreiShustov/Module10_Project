...
class Bank:
    ...
    def __init__(self, balance=0):
        ...
        self.add_transaction_id = 0
        self.withdraw_transaction_id = 0

    def deposit(self):
        for transaction in range(100):
            self.add_transaction_id += 1
            adding_summ = random.randint(50, 500)
            if self.balance >= 500 and self.lock.locked():
                self.lock.release()
            self.balance += adding_summ
            print(f'>> {self.add_transaction_id}. Пополнение: {adding_summ}. Баланс: {self.balance}')
            time.sleep(0.001)

    def take(self):
        for transaction in range(100):
            self.withdraw_transaction_id += 1
            withdrawing_summ = random.randint(50, 500)
            print(f'Запрос на {withdrawing_summ}')
            if withdrawing_summ <= self.balance:
                self.balance -= withdrawing_summ
                print(f'<< {self.withdraw_transaction_id}. Снятие:{withdrawing_summ}. Баланс: {self.balance}')
            elif withdrawing_summ > self.balance:
                print('Запрос отклонён, недостаточно средств!')
                self.lock.acquire()

            # если поток пополнений завершился, то разблокировать и дать завершиться потоку снятий
            if self.add_transaction_id >= 100 and self.lock.locked():
                self.lock.release()
