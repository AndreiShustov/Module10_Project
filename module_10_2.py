...
class Knight(threading.Thread):
    ...
    def __init__(self, name: str, power: int, number_of_enemies=100):
        super().__init__()
        self.name = name
        self.power = power
        self.number_of_enemies = number_of_enemies  # количество врагов

    def battle(self):
        enemy_remains = self.number_of_enemies  # остаток врагов
        day_number = 0

        while enemy_remains > 0:
            time.sleep(1)
            enemy_remains -= self.power
            day_number += 1
            print(f'{self.name}, сражается {day_number} день(дня)..., осталось {enemy_remains} воинов.')


        print(f'{self.name} одержал победу спустя {day_number} дней(дня)!')
